...
public class BinarySearchTree
{
    ...
    public void Add(int key)
    {
        ...
        while (cur != null)
        {
            if (key == cur.key && !cur.removed)
                throw new DuplicateKey();
            else if(key == cur.key && cur.removed)
            {
                cur.removed = false;
                HeIsBack = true;
                cur.subtreeSize++;
        ...
        while (prev != null)
        {
            if (prev.removed)
            {
                prev.subtreeSize = SubtreeSize(prev.right) + SubtreeSize(prev.left);
            }
            else
            {
                prev.subtreeSize = SubtreeSize(prev.right) + SubtreeSize(prev.left) + 1;
            }
            prev = prev.parent;
        }
    ...
    public void Remove(int key)
    {
        ...
        while (cur != null)
        {
            if (key == cur.key)
            {
                ...
                return;// marca este key como borrado
    ...
    private TreeNode FindMax(TreeNode NewRoot)
    {
        TreeNode cur = NewRoot, prev = cur;
        if (SubtreeSize(cur.right) < 1)
        {
            return prev;
        }
        else if (SubtreeSize(cur.right) >= 1)
        {
        ...
        else if (SubtreeSize(cur.left) >= 1)
        {
            return FindMax(cur.left);
        }
        return prev;
    }
    private TreeNode FindMin(TreeNode NewRoot)
    {
        TreeNode cur = NewRoot, prev = cur;
        if (SubtreeSize(cur.left) < 1)
        {
            return prev;
        }
        else if (SubtreeSize(cur.left) >= 1)
...
;
    ...
    public void Concatenate(BinarySearchTree other)
    {
        ...
        TreeNode OtherMin = other.MinNode(), ThisMax = this.MaxNode();
        if(OtherMin.key < ThisMax.key)
        {
            throw new InvalidOperation();
        }
        TreeNode cur = ThisMax;
        if(cur.parent == null)
        {
            cur.right = other.root;
            cur.subtreeSize = SubtreeSize(cur.right) + SubtreeSize(cur.left) + 1;
            return;
        }
        else
        {
           while(cur.parent != null)
            ...
            if (cur.right != null)
            {
                cur.right.right = other.root; // Si existe otro nodo pero no es el maximo significa que esta DEAD pero no nos olvidamos de el y lo movemos al nuevo arbol :D )
            }
            else
            {
                cur.right = other.root;
...
public class Lab4n
{
    ...
    public static void Main(string[] args)
    {
        ...
        Console.WriteLine("Valor minimo: {0} y valor maximo{1} ", tree3.Min(), tree3.Max());
        Console.ReadKey();
    }
}
